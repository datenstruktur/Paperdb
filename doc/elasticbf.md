...
In a filter unit indexed to index, to calculate the value generated by the hash function, LevelDB also uses double hashing, that is, to generate two hash values, and then generate k hash values based on the two hash values.
...
When the option of the multi_queue is empty, we will open the Table, directly from the disk to read data to create filterblockreader object, if not empty, indicating the need to filterblockreader by the multi_queue management, then we open the Table, from disk to create filterblockreader read data object is inserted into the multi_queue, while returning a handle containing the reader for query use.

When the table is released due to program exit or TableCache elimination mechanism, the reader saved in the table or the handle saved multi_queue will be released.
